...
func GenerateConfig(inFilenames []string, setFlags []string, force bool, client kube.Client,
	...
	return OverlayYAMLStrings("default", "", []string{}, force, client)
}

func OverlayYAMLStrings(profile string, fy string,
	setFlags []string, force bool, client kube.Client,
) (string, *iopv1alpha1.IstioOperator, error) {
	iopsString, iops, err := GenIOPFromProfile(profile, fy, setFlags, force, false, client)
	if err != nil {
		return "", nil, err
	}
...
spec:
  ...
  values:
    ...
    gateways:
      ...
      istio-egressgateway: {}
`
	finalIOP, err := unmarshalAndValidateIOP(outYAML, skipValidation, allowUnknownField)
	if err != nil {
		return "", nil, err
	}
	...
	// finalIOP.Spec.InstallPackagePath = ""
...
// error.
func unmarshalAndValidateIOP(iopsYAML string, force, allowUnknownField bool) (*iopv1alpha1.IstioOperator, error) {
